...
import static com.github.reviversmc.toomanybinds.TooManyBinds.LOGGER;
...
public abstract class LauncherScreen extends Screen {
	...
	public void render(DrawContext matrices, int mouseX, int mouseY, float delta) {
		...
		matrices.fill(getX()-1, getY()-1, getX()+w-1, getY()+lineHeight-2 + lineAmt * lineHeight, bgColor);
		...
		for (int i = optionOffset; i - optionOffset < TooManyBinds.config.maxSuggestions; i++) {
			...
			if (sg.favorite) {
				...
				matrices.fill(getX()-3, y-2, getX()-1, y+lineHeight-2, HIGHLIGHT_COLOR | bgColor);
			}

			// draw the bind name
			//drawTextWithShadow(matrices, textRenderer, sg.name, getX(), y, i == selected ? HIGHLIGHT_COLOR : SUGGESTION_COLOR);
			matrices.drawTextWithShadow(textRenderer, sg.name, getX(), y, i == selected ? HIGHLIGHT_COLOR : SUGGESTION_COLOR);

			// draw the bind category
			int textWidth = textRenderer.getWidth(sg.category)+2;
			matrices.drawTextWithShadow(textRenderer, sg.category, getX()+w-textWidth, y, SUGGESTION_COLOR);
		}
